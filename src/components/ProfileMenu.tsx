...
import { Settings, User, Palette, ChevronRight, Check, Trash2, Globe } from 'lucide-react';
...
export const ProfileMenu: React.FC<ProfileMenuProps> = ({ onNavigate }) => {
  ...
  const [activeSubmenu, setActiveSubmenu] = useState<'none' | 'theme' | 'language'>('none');
  ...
  const handleLogout = () => {
    if (confirm('Tem certeza que deseja limpar todos os seus dados de progresso?')) {
      localStorage.clear();
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        <img 
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = 'https://ui-avatars.com/api/?name=Taylor+Reis&background=random';
          }}
        />
      </button>
      ...
      {isOpen && (
        ...
        >
          {activeSubmenu === 'none' && (
            <div className="py-2">
              <div className="px-4 py-3 border-b mb-1" style={{ borderColor: theme.colors.border }}>
                <p className="text-sm font-bold truncate">Taylor Reis</p>
                <p className="text-xs opacity-60 truncate">github.com/taylorreis-lab</p>
              </div>
              ...
              >
                ...
                <span>Mudar Cor</span>
                <ChevronRight size={16} className="ml-auto opacity-40 group-hover:opacity-100" />
              </button>
          ...
          {activeSubmenu === 'theme' && (
            <div className="py-2">
              ...
              >
                ...
                {themeType === 'light' && <Check size={16} style={{ color: theme.colors.primary }} />}
              </button>
              ...
              >
                ...
                {themeType === 'dark' && <Check size={16} style={{ color: theme.colors.primary }} />}
              </button>
              ...
              >
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] opacity-60">Atual: {theme.name}</span>
                </div>
                {themeType === 'random' && <Check size={16} style={{ color: theme.colors.primary }} />}
              </button>
